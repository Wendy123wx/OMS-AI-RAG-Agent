...
- project-name（默认 `oara-web`）
- `{project-name}/docs/01-任务拆解报告.md`
- `docs/prd/02-领域模型.md`
...
1. 路由 `path` / `name` 与契约不一致，或页面里 `router.push` 写了另一套字符串。
2. 懒加载 `import()` 路径指向不存在的文件，或大小写与磁盘不一致。
3. 三个布局容器未正确嵌套：登录页 / 问答平台壳层 / 管理后台壳层与子路由 `router-view` 挂载错误，导致白屏或顶栏丢失。
4. 登录成功跳转、退出登录回 P01、「返回工作台」、Tab 切换（工作台 ↔ 历史记录 ↔ 管理后台）任一断链。
5. 普通用户仍能看见或进入管理后台路由；`meta.roles` 与全局 `beforeEach` 不一致（SEC-05）。
6. 动态路由或重复注册导致 Tab 切换丢失状态。

### B. 状态与鉴权

1. 多个 Store 描述同一概念但字段名不同（`userId` vs `id`，`token` 存法不同）。
2. 组件直接调 `api/*` 而不走 Store actions（STATE-04）。
3. 退出登录只清部分状态，问答消息/权限残留。
4. 会话消息被写入 localStorage（STATE-05 / AGENT-12）。
5. 登录 Mock 用户的 `role` 与路由守卫、顶栏「管理后台」显隐不一致。

### C. 类型与契约漂移

1. 同一领域对象在 `types/` 与 `features/*/types` 重复定义且不兼容（`QaSession` vs `QASession`）。
2. DTO/VO 混用：渲染层直接吃后端原始 DTO（AGENT-01）。
3. API 函数签名、Mock 返回、Store 使用的类型三者不一致。
4. 公开导出（`features/<module>/index.ts`）缺失或名称与引用方不一致（DIR-04）。

### D. 跨模块引用与重复实现

1. `features/*` 互相 deep import（DIR-02）。
...
1. 空态/错误态缺失导致接口失败白屏（ERR-01/03）。
2. `v-for` 使用 index 作 key（PERF-02）；魔法色值（STYLE-05）；`any`（TS-02）。
3. Element Plus 全局/局部注册混乱，样式丢失。
...
路径：`{project-name}/docs/03-README.md`

必须让陌生人能靠它跑起可体验前端，至少包含：

1. 项目简介（OARA 前端，基于哪些 PRD）
2. 技术栈
3. Node 版本与包管理器
4. 安装与启动命令（`pnpm install` / `pnpm dev` / `pnpm build`）
5. 环境变量说明（`.env.example`）
6. 默认体验账号（Mock：普通用户 / 管理员）
7. 页面与路由对照表
8. 目录结构说明（对齐 DIR-01）
9. 无后端时 Mock 行为说明
10. 已知限制

可同时在 `{project-name}/README.md` 写 3～5 行并链接到 `docs/03-README.md`，方便仓库根目录发现；`docs/03-README.md` 为完整正文（MUST）。
